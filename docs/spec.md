...
The database may store additional columns (raw payload JSON and retrieval metadata), but the API returns exactly this shape.
...
- `GET /api/v1/weight/latest` → fetches newly available provider data, then returns `{ "latest": WeightMeasurement | null }`
- `GET /api/v1/weight/list?days=30&provider=withings` → fetches newly available provider data, then returns `{ "measurements": WeightMeasurement[] }` — sorted by `measuredAt` desc; `days` default 30, max 365; `provider` optional filter
...
- `DELETE /api/v1/providers/:provider` → disconnect, delete stored tokens and internal retrieval state → `{ "ok": true }`
- `DELETE /api/v1/me/data` → delete all stored measurements, provider tokens, and internal retrieval state for the authenticated user → `{ "ok": true }`. This endpoint does not delete the account, sessions, API keys, limited inquiry, security, and backup records, or minimal legal acceptance records retained for a period reasonably necessary to verify contract formation and handle disputes.
...
- `DELETE /api/v1/me/data` is the measurement and connection data deletion contract. The backend deletes rows for the authenticated user from `body_measurements`, `oauth_tokens`, and `provider_refresh_state`. Account, session, API-key, and minimal legal acceptance record deletion are outside this endpoint; those legal acceptance records may be retained only for a period reasonably necessary to verify contract formation and handle disputes.
- `DELETE /api/v1/providers/:provider` deletes stored tokens and internal retrieval state for only the selected provider.
...
- **Withings**: OAuth connect, token refresh with locking, encrypted token storage, automatic data retrieval, and normalization are implemented and verified end to end on production `daylo.cc`.
- **Tanita Health Planet**: OAuth2 at `https://www.healthplanet.jp/oauth/auth` and `/oauth/token`, scope `innerscan`. Measurements: `GET /status/innerscan.json?tag=6021,6022` (6021 = weight kg, 6022 = body fat %). Dates are JST `yyyyMMddHHmmss` — convert to UTC. Production credentials are active; connection and data retrieval are verified end to end on production `daylo.cc`.
...
- `provider_refresh_state`: internal per-user/provider freshness and request-coalescing lease state
- `oauth_states`: state → (userId, provider, expiresAt) for the connect flow
- Weight read endpoints retrieve new provider data before querying normalized measurements. Tanita may reuse a successful retrieval for up to 60 seconds.
